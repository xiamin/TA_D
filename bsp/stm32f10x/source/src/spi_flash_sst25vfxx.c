...
#define MF_ID                       (0xEF)	//华邦0xEF(0xBF)
/* JEDEC Device ID : Memory Type */
#define MT_ID                       (0x40)		//华邦0x40	(0x25)
...
#define MC_ID_SST25VF016B            (0x15)	//(0x41) /* 16Mbit */
...
#define SPI_FLASH_WREN_PIN		GPIO_Pin_13
#define SPI_FLASH_WREN_PORT	GPIOC
...
static rt_size_t sst25vfxx_flash_write(rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
    ...
    if(result != RT_EOK)
    {
        ...
        GPIO_ResetBits(SPI_FLASH_WREN_PORT, SPI_FLASH_WREN_PIN);
        rt_thread_delay(1);
    ...
    for(i=0; i<size; i++)
    {
        ...
        GPIO_SetBits(SPI_FLASH_WREN_PORT, SPI_FLASH_WREN_PIN);
    /* release lock */
    rt_mutex_release(&(spi_flash->lock));
