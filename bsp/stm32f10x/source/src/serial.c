...
static rt_size_t rt_serial_write (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
	...
	uart = (struct stm32_serial_device*)dev->user_data;
       ...
       if (uart->ComType == ComType_485)
      {
          GPIO_SetBits(uart->DirPort, uart->DirPin);
           //Delay_1ms(5);
      }
	...
	else
	{
       ...
       if (uart->ComType == ComType_485)		//485端口
       {
           //Delay_1ms(5);
	    while (!(uart->uart_device->SR & USART_FLAG_TC));
           GPIO_ResetBits(uart->DirPort, uart->DirPin);
       }
...
static rt_err_t rt_serial_control (rt_device_t dev, rt_uint8_t cmd, void *args)
{
	...
	switch (cmd)
	{
	case RT_SERIAL_CMD_ClearRxBuff:
	     	if (dev->flag & RT_DEVICE_FLAG_INT_RX)
	     	{
	     		rt_memset(uart->int_rx->rx_buffer, 0,
	     			sizeof(uart->int_rx->rx_buffer));
       ...
       case RT_SERIAL_CMD_SETBPS:
            #if 1
            {
                ...
                char Str[5];
                uint32_t Bits;
                sscanf((char *)args, "%d-%s-%d", &Br, &Str, &Bits);
                switch (Str[0])
                {
                case 'O':
                ...
                #if 0
                Br = simple_strtoul((const char *)args, pStr, 10);
                if (*pStr == '-')
                    pStr++;
                #endif
                uart_SetBps(uart->uart_device, Br, Parity);
            }
            #endif
            break;
