...
#define PORT_485_DIR	GPIOB
#define PIN_485_DIR		GPIO_Pin_9
...
static void GPIO_Configuration(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = PIN_485_DIR;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(PORT_485_DIR, &GPIO_InitStructure);  

       GPIO_ResetBits(PORT_485_DIR, PIN_485_DIR);
#endif
}
...
void uart_SetBps(USART_TypeDef *Port, uint32_t br, uint16_t pariMode)
{
    ...
    /* Disable the USARTx */
    USART_Cmd((USART_TypeDef *)Port, DISABLE);
    USART_StructInit(&USART_InitStructure);

    USART_InitStructure.USART_BaudRate = br;
    USART_InitStructure.USART_Parity = pariMode;
    if(pariMode == USART_Parity_Even ||pariMode == USART_Parity_Odd)
    {
...
void rt_hw_usart_init()
{
	...
	/* register uart3 */
	rt_hw_serial_register(&uart4_device, "485",
		RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_STREAM,
		&uart4);
