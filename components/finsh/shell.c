...
/*
  *	1.分析输入的字符串，将其分割成一个一个的词法单元
  	   并构造成树形结构，每个节点为1个词法单元
...
void finsh_run_line(struct finsh_parser* parser, const char *line)
{
    ...
    rt_kprintf("\n");/*首先换行*/
    finsh_parser_run(parser, (unsigned char*)line);	/*编译原理中的扫描程序(构造语法树)*/
...
void finsh_thread_entry(void* parameter)
{
    ...
    {
        /* wait receive */
        if (rt_sem_take(&shell->rx_sem, RT_WAITING_FOREVER) != RT_EOK) continue;/*continue跳过此循环的剩下部分*/
        ...
        while (rt_device_read(shell->device, 0, &ch, 1) == 1)
        {
...
#ifdef FINSH_USING_HISTORY
                finsh_push_history(shell);	/*将输入的命令往history数组中push*/
#endif

                if (shell->line_position != 0) finsh_run_line(&shell->parser, shell->line);	/*开始解析*/
                else rt_kprintf("\n");
            ...
            if (shell->line_position >= FINSH_CMD_SIZE) shell->line_position = 0;	/*命令过长，忽略*/

            /* normal character */
            shell->line[shell->line_position] = ch;
            ch = 0;
            if (shell->echo_mode) rt_kprintf("%c", shell->line[shell->line_position]);/*回显单个普通字符*/
            shell->line_position ++;
