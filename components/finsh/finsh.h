...
struct finsh_token
{
    char eof;				/*用于记录词法单元是否结束*/
    char replay;				/*表示正在解析的词法单元是否需要重新解析*/

    int  position;			/*记录正在解析词法单元中的哪个位置*/
    u_char current_token;	/*用于记录当前解析词法单元的类型*/
    ...
    union
    {
        ...
        int int_value;
        long long_value;
    } value;				/*用于记录当前解析词法单元的值*/
    u_char string[128];		/*用来存储id型的词法单元*/
...
struct finsh_parser
{
    u_char* parser_string;	/*用于指向需要处理的字符串*/
    struct finsh_token token;	/*编译原理中的记号*/
    struct finsh_node* root;	/*编译原理中的语法树*/
};
